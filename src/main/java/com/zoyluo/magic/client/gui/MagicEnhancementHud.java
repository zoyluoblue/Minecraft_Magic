...
public final class MagicEnhancementHud {
	...
	public static List<DisplayEntry> collectDisplayEntries(PlayerEntity player) {
		...
		addStackLines(entries, "mainhand", "slot.magic.mainhand", player.getEquippedStack(EquipmentSlot.MAINHAND));
		addStackLines(entries, "offhand", "slot.magic.offhand", player.getEquippedStack(EquipmentSlot.OFFHAND));
		addStackLines(entries, "head", "slot.magic.head", player.getEquippedStack(EquipmentSlot.HEAD));
		addStackLines(entries, "chest", "slot.magic.chest", player.getEquippedStack(EquipmentSlot.CHEST));
		addStackLines(entries, "legs", "slot.magic.legs", player.getEquippedStack(EquipmentSlot.LEGS));
		addStackLines(entries, "feet", "slot.magic.feet", player.getEquippedStack(EquipmentSlot.FEET));
		return entries;
	}
	...
	private static void addStackLines(List<DisplayEntry> entries, String slotKey, String slotTranslationKey, ItemStack stack) {
		if (stack.isEmpty()) {
			return;
		}
		...
		for (EnhancementType type : EnhancementSystem.getApplicableTypes(stack)) {
			...
			if (!level.isEmpty() && hasDisplayValue(type)) {
				Text slotLabel = Text.translatable(slotTranslationKey);
				entries.add(new DisplayEntry(slotKey + ":" + type.id(), slotKey, slotLabel, Text.translatable(type.translationKey()), createLine(slotLabel, stack, type, level)));
			}
		}
	}

	private static boolean hasDisplayValue(EnhancementType type) {
		return switch (type) {
			case WATER_SOUL, FIRE_SOUL -> ClientSoulChargeCache.hasAuthoritativeValue(type);
			default -> true;
		};
	}

	private static Text createLine(Text slotLabel, ItemStack stack, EnhancementType type, EnhancementSystem.Level level) {
		return Text.translatable(
				...
				level.display(),
				getValueText(stack, type)
		);
	}

	private static Text getValueText(ItemStack stack, EnhancementType type) {
		return switch (type) {
			case WATER_SOUL, FIRE_SOUL -> Text.translatable("value.magic.remaining_seconds", ClientSoulChargeCache.getRemainingSeconds(type));
