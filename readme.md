...
- **玩家自行选择 HUD：** 按 `F9` 勾选要显示的装备数值和探矿目标，减少不需要的信息。
...
6. 查看物品 Tooltip，或按 `F9` 选择需要显示在 HUD 中的强化信息。
...
当前阶级色用于强化台星环。装备在背包、手持、掉落或穿戴状态下的全场景阶级辉光尚未发布；它也不会自动照亮环境。头盔“照明”是独立强化效果。

## HUD 与射钉枪操作

| 操作 | 默认按键 | 说明 |
| --- | --- | --- |
| 打开 HUD 选择页 | `F9` | 可在 Minecraft 按键设置中重绑 |
| 发射/释放射钉枪 | `Alt + 1` | 仅在未打开界面且装备射钉枪胸甲时读取 |
| 开始牵引 | 当前跳跃键，默认 `Space` | 锚定后按一次开始持续牵引 |
| 释放射钉枪 | 再按 `Alt + 1` 或 `Esc` | 到达、受阻、超时、死亡或换维度也会自动释放 |

`F9` 页面按主手、副手、头盔、胸甲、护腿和鞋子分组。玩家自行勾选后，左下角显示装备位置、物品名、强化类型、等级和数值；探矿指示位于屏幕上方。HUD 与探矿选择只保留在当前客户端会话，重启后需要重新选择。
...
目前只有强化台星环按蓝、紫、粉、橙四阶变色。装备全场景阶级辉光尚未发布，也不会作为动态光源；头盔“照明”是独立功能。
...
正式构建会执行 Java 编译、资源处理、JAR 打包、中英文 translation key 一致性检查和原创星环强化台资源门禁。当前 Dedicated Server 套件包含 34 项 GameTest，覆盖材料事务、持久化、战斗、远程武器快照、临时效果、射钉枪状态机与强化台视觉状态契约。
