...
1. macOS 先按住键盘左侧的 `Option (⌥)`；Windows 先按住键盘左侧的 `Alt`。
2. 保持左侧修饰键不放，再按 HUD 主键（默认 `B`），即可打开 HUD 选择页。必须使用左侧 `Option/Alt`；单按 `B`、右侧 `Option/Alt` 或旧版 `F9` 都不会打开。
3. 勾选要显示的装备部位、强化数值和探矿目标，然后点击“确认”。装备信息会显示在左下角，探矿箭头与距离会显示在右上角。
4. 如需更换 `B`，进入 Minecraft“设置 → 按键控制”，找到“选择强化数值显示（需按左 Option/Alt）”并重绑主键；左侧 `Option/Alt` 修饰键固定，不能重绑。

> 从 `2.2.0` 或更早版本升级时，如果曾自定义旧 `F9` 按键，需要在新的按键设置项中重新设置一次。
