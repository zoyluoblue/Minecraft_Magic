...
正式构建会执行 Java 编译、资源处理、JAR 打包、中英文 translation key 一致性检查，以及原创星环强化台和轮廓 Shader 资源门禁。当前 Dedicated Server 套件包含 35 项 GameTest，覆盖材料事务、持久化、战斗、远程武器快照、临时效果、射钉枪状态机与强化台/轮廓视觉规则。
